...
class DMOJLoginMiddleware(object):
    ...
    def __call__(self, request):
        ...
        request.official_contest_mode = settings.BKDNOJ_OFFICIAL_CONTEST_MODE
...
class OrganizationSubdomainMiddleware(object):
    ...
    def __call__(self, request):
        ...
        if subdomain.isnumeric() or subdomain in settings.BKDNOJ_IGNORED_ORGANIZATION_SUBDOMAINS:
            return self.get_response(request)
