...
class ProfileForm(ModelForm):
    ...
    class Meta:
        ...
        if settings.BKDNOJ_OFFICIAL_CONTEST_MODE:
            fields.remove('about')
...
class LanguageLimitForm(ModelForm):
    ...
    def clean_time_limit(self):
        ...
        if timelimit and timelimit > settings.BKDNOJ_PROBLEM_TIMELIMIT_LIMIT and not has_high_perm:
            raise forms.ValidationError(_('You cannot set time limit higher than %d seconds')
                                        % settings.BKDNOJ_PROBLEM_TIMELIMIT_LIMIT,
...
class ContestForm(ModelForm):
    ...
    def clean(self):
        ...
        if end_time and start_time and \
           (end_time - start_time).days > settings.BKDNOJ_CONTEST_DURATION_LIMIT and not has_long_perm:
            raise forms.ValidationError(_('Contest duration cannot be longer than %d days')
                                        % settings.BKDNOJ_CONTEST_DURATION_LIMIT,
