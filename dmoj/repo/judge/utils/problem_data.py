...
def get_visible_content(archive, filename):
    if archive.getinfo(filename).file_size <= settings.BKDNOJ_TESTCASE_VISIBLE_LENGTH:
        data = archive.read(filename)
    else:
        data = archive.open(filename).read(settings.BKDNOJ_TESTCASE_VISIBLE_LENGTH) + b'...'
