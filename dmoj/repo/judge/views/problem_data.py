...
class ProblemDataView(TitleMixin, ProblemManagerMixin):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.has_perm('judge.create_mass_testcases'):
            ...
            context['testcase_limit'] = settings.BKDNOJ_TESTCASE_HARD_LIMIT
            context['testcase_soft_limit'] = settings.BKDNOJ_TESTCASE_SOFT_LIMIT
    ...
    def check_valid(self, data_form, cases_formset):
        ...
        if number_of_cases > settings.BKDNOJ_TESTCASE_HARD_LIMIT and \
           not self.request.user.has_perm('judge.create_mass_testcases'):
            error = ValidationError(
                _('Too many testcases, number of testcases must not exceed %s') % settings.BKDNOJ_TESTCASE_HARD_LIMIT,
