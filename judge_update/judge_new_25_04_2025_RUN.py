    ...
    def _grade_cases(self) -> Generator[Tuple[IPC, tuple], None, None]:
        ...
        if sample_testcase_only:
            ...
            if not sample_input_files:
                # No sample files specified - run nothing
                logger.warning('sample-testcase-only=True but no sample-input-files provided, skipping all cases')
            ...
            if not filtered:
                logger.warning(
                    'sample-testcase-only=True but no cases matched sample-input-files %s, skipping all cases',
                    sample_input_files,
                )
                yield IPC.GRADING_END, ()
                return
        ...
        is_partial_testcase = self.submission.scoring_mode == 'partial_testcase'
        ...
        for batch_number, cases in groupby(flattened_cases, key=itemgetter(0)):
            if batch_number:
                ...
                if not is_partial_testcase:
                    # In partial_testcase mode, never skip a batch due to failed dependencies.
                    # All batches must be fully graded.
                ...
                if is_short_circuiting:
                    ...
                    if result.result_flag & Result.WA:
                        if is_partial_testcase:
                            # In partial-testcase mode: NEVER short-circuit.
                            # Every test case in every batch is always graded.
                            # Points for the batch = sum of points of correct tests.
                            pass
                        else:
                            # If we failed a 0-point case, we will short-circuit every case after this.
                            is_short_circuiting_enabled |= not case.points

                            # Default behavior: short-circuit within batch on failure,
                            # or globally if short-circuiting is enabled.
